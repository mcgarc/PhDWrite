...
It is common to describe resonators and waveguides in terms of a quality
factor~\cite{1125652}
\begin{equation}
  Q = \omega \frac{\text{Energy stored per unit length}}{\text{Power loss}}.
\end{equation}
For our purposes we consider the enrgy stored to be the energy that we keep in
the waveguide after traversing some distance $L$. Taking this to be the energy
contributed by the electric field, this is
\begin{equation}
  \frac{\epsilon_0}{2} \lvert A(L) \rvert^2.
\end{equation}
The power loss is then all the dissipated power
\begin{equation}
  \epsilon_0 \left[ \lvert A(0) \rvert^2 - \lvert A(L) \rvert^2 \right] c/L.
\end{equation}
We therefore have quality factor
\begin{equation}
  Q = \frac{(2\pi/\lambda)}{e^{2\alpha} -1}
\end{equation}
where we have taken $L=\SI{1}{\meter}$, so that $\alpha$ is expressed in Neper
per meter and $\lambda$ in metre. Note that for low loss ($\alpha \ll1$) this
can be expanded for
\begin{equation}
  Q = \frac{\beta}{2 \alpha}
\end{equation}
...
This corresponds to $Q\sim10^{-50}$. It appears the conductor losses are going
