...
\label{litrev:molculechips}
 ...
 chip.  \cm{talk a bit about what happened in this experiment -- molecules fly
 over the surface}
...
experiment~\cite{Bethlem1999}. This eventually allowed slowing of molecules to a
standstill 

On-chip control of the molecules was demonstrated by the same group, using
microwaves propagating through free space.~\cite{doi:10.1002/cphc.201001007}
\cm{summarise results}

These chips still lacked the ability to measure the molecules in situ, with
molecules being accelerated off the chip for measurement. Whilst being a clear
demonstration of the potential of molecule chips, the lack of on-chip
measurement and control mean that these chips are inherently not scalable.
Furthermore, the trapping volumes were still fairly large, around
\SI{0.25}{\milli\metre\cubed}. \cm{Why do we want a small trapping volume?}
...
Recently, magnetically insensetive transitions have been discovered in \cm{CaF}.
~\cite{PhysRevLett.120.163201, Blackmore_2018, } \cm{need one more cite for
Luke's new paper (unpublished???)} One transition between the \cm{insert states}
has been found at \cm{frequency} and another between \cm{other states} at
\cm{40GHz}. The \cm{20GHz} transition is shown to be \cm{quite insensetive} and
the \cm{40GHz} to be \cm{VERY insensetive}, leading to fidelities over
\cm{length of time}.
