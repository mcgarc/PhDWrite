...
vibrational level $v=$ \numrange{0}{3} respectively.  For the X state, we
will discuss $N=$ \numrange{0}{2}, but $N=2$ is not included in this figure for
clarity. 
%
...
and also cooled to \SI{4}{\kelvin} is used as a helium
absorber~\cite{doi:10.1116/1.574141}. These are theramlly cycled overnight when
the source is not in use to avoid saturation.  A mechanical shutter is also
installed after the cell to minimise any increase in background pressure due to
\He{} gas leaving the source.
...
\pewpew{S}{00} slowing light. This is chirpped to account for the change in
...
transverse scattering~\cite{Truppe20717a}. 
...
time the light is not chripped, but has the r.f.\ sidebands to address the $X$
hyperfine structure. The slowing procedure described above is carried out
...
\pewpew{}{32} light is incident on the molecules in combined beams entering on
all three axes and retroreflected to provide restoring beams in both directions
(see \myfigref{overview:fig:cartoon}).
%
...
molecules will be pumped into these dark states. All of these beams must havei
...
\pewpew{}{00}. The MOT temperature is typically \cm{}, and this can be further
reduced by lowering the intensity of \pewpew{C}{00}~\cite{Truppe2017} to
achieve a MOT temperature of $<\SI{4}{\milli\kelvin}$.

\subsection*{Optical molasses}

Sub-Doppler cooling of \CaF{} is achievable by employing a blue-detuned
molasses. In this scheme the MOT coils are switched off, and \pewpew{C}{00} is
now blue-detuned from the transition frequency. Consider again the example of a
$F=1$ hyperfine level ground state, and $F=0$ excited state travelling in 1D,
as is shown in \myfigref{overview:fig:molasses}.  The counter-propagating beams
establish a polarisation-gradient, which coulple the hyperfine states in $F=1$
to the $F'=0$ state. As explained in \inlineref{Weidem_ller_1994} we will have bright
...
\begin{figure}
  \centering
  \cm{fig like Hannah's 5.1}
  \caption{}
  \label{overview:fig:molasses}
\end{figure}

When a molecule in a dark state moves through the potential it can expend lose
climbing the potential. At the top of the potential it is prefferentially
...
hyperfine state~\cite{1367-2630-18-12-123017, PhysRevLett.123.033202}. In our
experiment we apply this technique to produce a \CaF{} cloud of temperature
...
\inlineref{WilliamsMagnetic2018}. This transfer is achieved with optical
pumping, where the EOM producing the \pewpew{}{00} sidebands is over-driven,
reducing the power in the sideband that addresses $F=0$. Hence the molecules
are preferentially moved into $\ket{N=0, F=0, m_F=0}$. These molecules can then
be transferred to weak-field seeking states by a microwave pulse, and any that
were not succesfully pumped can be blown away by \pewpew{S}{00} light.
...
\inlineref{WilliamsMagnetic2018}. We can transfer them to the MTT for transport
to neighbouring experiments, or hold them in the field generated by the MOT
coils for experiments in the collisions chamber.
...
\inlinerefs{Lewandowski2003,PhysRevResearch.1.033035} and elsewhere. In the
next section, we will begin to discuss how the molecule chip can also be
incorporated as an additional experiment.
...
such as those describedin \cm{ref theory section} would be preferable to the
...
trap which is aligned to the chip and a large target for loading form the MTT.
This will be discussed further in \cm{experiment chapter}.
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \includegraphics[height=\textwidth]{figs/overview/chamber_xsecarrow.pdf}
    \caption{}
  \end{subfigure}
  \hspace{1cm}
  \begin{subfigure}[b]{0.45\textwidth}
    \includegraphics[height=\textwidth]{figs/chip_pic_crop.png}
    \caption{}
  \end{subfigure}
  \caption{
...
subchip for current and microwave delivery. The chip is mounted
...
fall. The flange itself is fitted with two high-current feedthroughs, a 16-pin
feedthrough rated for \SI{3}{\ampere} currents, and microwave feedthroughs. All
access to the chip is therfore through this one flange, allowing for easy access
and assembly. The flange assembly will be explained in futher detail in
chapter~\ref{experiment}.
