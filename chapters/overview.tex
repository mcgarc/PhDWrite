...
as shown (in cross section) in \cm{fig}. Molecules can be brought in along the
transport axis and positioned below the surface of the chip, which is mounted
on the chip flange assembly, detailed in \cm{fig}.
...
delivery. The \cm{science chip} is mounted into a recess in the subchip so that
...
with two high-current feedthroughs, a \cm{??} pin feedthrough rated for
\cm{currents}, and microwave feedthroughs. In this way the entire assembly 
is \cm{self contained and awesome}.
