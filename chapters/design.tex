This chapter will describe the design of the chip and the supporting
infrastructure for integration into the existing \CaF{} experiment. Designing
the chip has been an iterative process, which has been motivated by analysis of
the trapping fields, trajectory simulations and constraints encountered in the
fabrication process.

Here we will largely discuss the analysis of the trapping fields and the
trajectory simulations. These two factors have been used to determine the major
structure of the chip. Changes introduced due to fabrication limitations will
be presented here, with details in chapter~\ref{fab}.

\section{Initial design}

The chip trap must integrate into the existing \CaF{} experiment, which has
been described \ph{above and in \cite{Truppe2017}, as well as others}. We
\ph{have recently developed (with cite ?)} an optical tweezer, which could be
used for loading the chip as in \inlinerefs{??}, however this was not available
at the start of the project. As such it was decided that the molecules would be
transferred between magnetic traps onto the chip.

It would be immensely difficult to bring the molecules from the transport trap
directly onto a microscopic trap on the chip \cm{cite or further discussion?}.
We therefore decided to use a series of ramps to transfer the molecules into
the microscopic trap. For trapping further from the chip, where the molecule
cloud will be brought in, a higher current is required and correspondingly a
wider wire. It is undesirable to use a wide wire for the smallest traps, as we
aim for the current to be localised on a length scale that is small compared to
the cloud. \cm{cites?}

Another difficulty is alignment of the macroscopic trap with the chip traps. To
overcome this we have incorporated an intermediary trapping \cm{U-wire} trap,
made out of thick copper and positioned directly beneath the chip. This trap
will be aligned with the chip by virtue of the design, and the transport trap
need only be aligned with the transport trap with the larger intermediary \cm{U
trap}.

% Overlapping wire traps to avoid shifting the trap centre

% Back of the envelope calculations for wire heights, fields, relative trap
% heights

% Integration with microwave guides

Using these principles, we created our initial design (hereafter referred to as
`design 1') shown in \cm{figure TODO}. We hoped that this design would be close
enough to the final design to allow us to begin honing our fabrication
techniques. It was not clear if the fuse-like nature of this design would allow
the high-current 

\section{Supporting infrastructure}

% Setup: we want to be able to do the stuff we discussed for intial chip design

% Present idea of flange assembly, break down the parts
%  - Large U
%  - Subchip
%  - Feedthroughs (chip, large current and mw)

\section{A refined chip design}

% Present design 2

% Discussion of loading scheme, inc. simulations

\section{Experiment overview}

\cm{Needs to complement discussion in intro, don't want to refer back and
forth.}
...
\cm{Try to recreate this fig myself with my code!}
%
\begin{figure}
  \centering
...
\cm{I should discuss filamentation in more detail}
...
\cm{This sounds like a pretty weak excuse. I better figure out what is
...
$443/10^{6}\times$ initial phase space volume. It would be interesting to compare
...
\cm{Check that the separatrix is calculated sensibly, is it the same in
every direction?}
%
\begin{figure}
  \centering
...
\cm{In these figures state the trapping heights $z_0$}
%
\begin{figure}[p]
\centering
  \begin{tabular}{c}
