...
\end{equation}
%
% NOTE it is really 3.4 10^-12 PSD in paper. BS told me typical N ~ 10^4
...
\myfigref{design:fig:psaeg} (a) and (d), where the trap has been simulated for
...
\end{equation}
where $B'$ is the field gradient in the $z$ direction. \cm{This eqn. will probably end up in an earlier
chapter.}
...
in each direction as defined in \myfigref{design:fig:wiresegment}. We also have
...
increasing trap gradient compresses the distribution.

% TODO Fig here showing init distro
...
% TODO Check temperature here, said init at 5uK above
...
\SI{3}{\milli\meter}. The ramps are summarised in \myfigref{design:fig:simsum}
and the simulation is presented in \myfigref{design:fig:uzsim}.

In this figure we have the initial (a, blue) and final (b, red) particle distributions.
With the $z$ velocities and positions summarised in the histograms in (c) and
(e) respectively. We can see that the mean position and velocity remains the
same, but it is clear from (b) that there are a number of particles that have
picked up velocity in the positive $z$ direction, and can be seen leaving the
trap. This is the notable streak to the right of the subfigure, and can also
be seen in the tail of the histogram in (e). The particle number decreases, as
seen in (f). This loss is similar to that of the MTT to U-trap handover, with a
decrease in trap depth that we attribute from changing from a quadrupole to
Ioffe-Pritchard trap. This changeover can be seen in (d) where we note that the
final trapping potential's minimum is non-zero.
...
\centering
  ...
  \caption{
    ...
    shaded grey (c.f.  \myfigref{design:fig:simsum}).
  }
...
\centering
  ...
  \caption{
    ...
    in \myfigref{design:fig:mttusim}. The particle loss before the ramp
