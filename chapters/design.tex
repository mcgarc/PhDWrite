...
\cm{magnetic dipole moment} of the molecule.  The motion of any one particle is
...
example is shown in \myfigref{design:fig:acceptance}.
...
are trapped and which are not \cm{(as in the simulation part of the example
above?)}. In our case the loading is achieved by a series of adiabatic ramps,
...
result in an increase in phase space density.~\cite{} \cm{Big citation needed
on this one!} This will be heavily exploited when transferring between Z wire
...
at $\mathbf{q}$, as is shown in \myfigref{TODO}. The field
