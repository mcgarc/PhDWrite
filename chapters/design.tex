...
\end{equation}
%
% TODO Check that these parameters are not introudced somewhere else earlier.
% especially mass
...
equation~\ref{intro:eq:trapdepth}. The \CaF{} mass is
$m_\text{\CaF{}}=\SI{9.79E-26}{\kilogram}$.  We therefore expect that as many
as $10^5$ molecules can be trapped.
...
$z=\SI{3}{\milli\meter}$, as shown in \myfigref{design:fig:initsum}.
...
\centering
  ...
  \caption{
    ...
    (c.f.~\myfigref{design:fig:simsum}).
  }
...
\mysubfigref{design:fig:simsum}{d}). This loss is similar to that of the MTT to
...
\myfigref{design:fig:phasematchinggrid}. Performing the handover adiabatically
will maximise the number of molecules that can be transfered.
...
The particles will undergo fillamentation, exploring all regions that are
energetically accesible and increasing the effective phase-space volume that
...
\mysubfigref{design:fig:phasematchinggrid}{c}.
...
effective phase-space density by fillamenation, which will further reduce the
