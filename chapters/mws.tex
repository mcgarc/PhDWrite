...
CPW in \myfigref{mws:fig:implement}. Note that the orientation of the waveguide
relative to the trapping wires is not important for driving transitions (the
molecules' orientation is not determined by the trap geometery or relevant to
the incoming field direction) but this orientation is chosen with the hopes
that any bumps caused by the wires will not unduly affect the waveguide
properties.
...
\end{subfigure}
  ...
  \caption[Microwave components of the flange assembly]{
    ...
    \cm{Comment somewhere (in body) on the fact that alignment of CPW doesn't
      matter for the transition -- relates to dipole moment qn that I can't
      answer.}
  }
