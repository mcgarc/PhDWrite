...
cloud of molecules trapped nearby.  As a rough approximation, this is achieved
when the molecules are trapped above the centre of the CPW, at a distance on the
...
are calculated analytically for arbitrary parameters. In both subfigures the even
...
It is also possible to match between macroscopic and microscopic components by
...
\beta^*\bra{\beta}$ to remove the $b$ and $b^\dagger$ operators and replace
...
\end{equation}
%
The detuning required for our $g$ and $Q$ is therefore $\Delta \gg 2\pi \times
\SI{5}{\mega\hertz}$, which is within the bounds achievable with the Stark
shift.
...
The entangling of the molecule and light state will produce a powerful system
...
$i^\text{th}$ molecule. Such an interaction can be used to
