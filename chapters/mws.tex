...
I will begin with a summary of the coplananr waveguide (CPW), which can be used
to guide microwaves across a dielectric surface. I will then move on to discuss
the logistics of integrating a CPW into the experiment, and espeically on the
...
even if $h$ is chosen to be several microns.
...
impolementation including the detuning of molecules with Stark shift, the
...
rotational state $\ket{N, m_N}_\text{rot}$
% TODO Leave this note for Mike, but also have to do it!
\cm{need to intoduce this notation in theory section}
is given by 
%
...
The subchip CPW will be connected to the science chip CPW by wire-bonds.
...
% TODO Note mainly for Mike, need to do it!
\cm{Need to introduce the stretched state notation in theory as in long
  coherence times... paper}
...
states. In this case the resonator is populated with photons, so that the
molecule-resonator coupling is then described by the Jaynes-Cummings
Hamiltonian described in section~\ref{theory:QO}.
%
...
the interaction of the photons and the spins induces the usual AC start shift
proportional to $(n+\frac{1}{2})$. The last term allows the pefromance of QND
