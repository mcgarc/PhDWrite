...
the molecule to act as a two-level system in the stretched states $\ket{g} =
??$ and $\ket{e}=??$.
...
We again assume that the molecule is trapped in one of the stretched states
$\ket{g}=??$ and $\ket{e}=??$. The molecule-resonator coupling is then
described by the Jaynes-Cummings Hamiltonian described in
section~\ref{theory:QO}.
%
...
$a'=\sqrt{\kappa}a$~\cite{Zhang2019}.
%
% TODO TODO Need a different symbol from kappa because this is already used!
% Also fix below
%
...
%
\cm{I think this is off by a little bit, see coupling notebook}
\begin{equation}
