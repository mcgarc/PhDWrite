...
$t=\SI{5}{\micro\meters}$. In both the free parameters are chosen to match an
...
the dielectric losses. For this reason it is common to use superconductors for
CPWs~\cite{}, since for a superconductor $\sigma \to \infty$ and hence $R_s$
and $\alpha_c \to 0$.  This will not prevent the implementation of a microwave
guide on a multi-layer chip trap, but it will impact our ability to build
incorporate a microwave resonator as we will now discuss.
...
experiments~\cite{WilliamsMagnetic2018} although realisation of the
% TODO Maybe this is just JCH here? Could probably ref eqn once I sort out the
% theory...
Jaynes-Cummings Hamiltonian will
...
find that without the use of superconductors, the attenuation will be dominated
...
including tuning the molecular rotational frequency using the Stark shift,  the
