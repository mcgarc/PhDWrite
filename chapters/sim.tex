...
field\footnote{This means that simulations investigating fast-changing fields
  ...
  the molecules (which is longer than the dipole alignment). Hence this
adiabatic approximation is sufficient for our investigations.}, so that
$V=\mathbf{\mu}\cdot\mathbf{B}\approx\mu B$.  The potential is therefore
%
...
transfer and the timescale required to be adiabatic with respect to the motion
of the molecules.
...
\myfigref{sim:fig:simsum}. Note that throughout the rest of this chapter we say
that the chip is situated at a height $z=0$.
