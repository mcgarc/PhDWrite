...
% TODO Need loads of cites here.

% TODO TODO TODO This is mostly wrong, which you can see if you actually read
% the quantum systems notes
...
Typically when the transitions are between electronic states, we can also have
a change in the vibrational level $v$. We will represent this graphically as
shown in \cm{figure}. As mentioned already, the electronic potentials are found
computationally, and for our purposes can be said to take  the form of
\cm{Morse potentials}, so that
%
\cm{eqn.}
%
The vibrational energy can then be thought of as a level in this potential,
which we can treat like an anharmonic oscillator, having a wavefunction
$\psi_\text{vib}(q, v; R_{AB})$, as depicted in the figure. The strength of the
transition between any two such states is then proportional to the mod-square
of the overlap integral of these functions. This is called the Franck-Condon
factor and is written
%
\cm{FC factor.}
%
In the case that $q=q'$, the vibrational states are orthogonal except when
$v=v'$. Any such transitions within the same electronic state must therefore be
purely rotational.
