...
% Junk that I want to commit in case I want it at some point in the future...

%$D^{(1)}(\Theta, \Phi)$,
%so that
%%
%\begin{equation}
%  d_r = \sum_s D^{(1)\dagger}_{r,s}(\Theta, \Phi) \tilde{d}_s
%\end{equation}
%%
%where $\tidle{d}_q$ is the rotated dipole moment. By substitution of the above
%we therefore have
%%
%\begin{equation}
%  d_{eg} = \sum{r}\epsilon_r\sum_s M_{r,s} \bra{q', v'}\mu_s\ket{q, v}.
%\end{equation}
%%
%Where the factor
%%
%\begin{equation}
%  M_{r,s} = \bra{R', m_R'} D_{r,s}^{(1)}\ket{R, m_R}
%\end{equation}
%%
%represents all change in the rotational state, and contains all dependence on
%the angular coordiante.
%
...
%axis. Re-writing \myeqref{theory:eqn:dge} in terms of the spherical basis unit
%vectors $\hat{e}_p$ we have
%%
%\begin{equation}
%  d_{ge} = \sum_p \bm{\epsilon}\cdot \hat{e}_p  \text{etcetera}
%\end{equation}
