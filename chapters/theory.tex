...
lowers the energy of the $m_F = 1$ ($m_F=-1$) state in the positive (negative)
$x$ direction according to the usual relationship
%
\cm{Zeeman equation}
%
...
% TODO Need to fill in this figure, there are some changes, like I'm not using
% wavy arrows for decays, and I think the polarisaitons need to be superscripts
% in the fig to match the text...
\begin{figure}
  \cm{Figure from presentation with changes for consistency}
  \caption{}
...
trap. Particles are preferentially excited when their energies are higher, and
will preferentially decay into the ground states. So there is an optical
pumping from the raised state into the lower state. Once in the lower state,
the particle will follow the potential curve until it is pumped again. The
particles on average spend more time climbing the potential than descending it,
and therefore expend energy through this process, reducing their temperature.
