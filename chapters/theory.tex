...
%
\begin{equation}
  \psi_n(\mathbf{R}_{AB}) = \frac{1}{R_{AB}}f(R_{AB})Y_{N, M_N}(\theta, \phi)
  \label{theory:eqn:nucsep}
\end{equation}
%
where we choose the factor of $1/R_{AB}$ to simplify the equation in the rest
of the section. We will introduce the operator $\mathbf{N}$ to describe the
rotational angular momentum. The angular part of the solution is given by the usual
spherical harmonics $Y_{N, M_N}$ where $N$ is the rotational quantum number for
...
interaction with $\mathbf{N}$, so the final good quantum numbers are $J$ and
$S$.

In the second case of interest (b), $L$ is strongly coupled to the axis as
before, but it then couples to the rotational operator $\mathbf{N}$ to form
$\mathbf{N} = \mathbf{L} + \mathbf{N}$ % TODO fix the symbols here
which will couple to $\mathbf{S}$. Here the total angular momentum operator is
$\mathbf{J} = \mathbf{N} + \mathbf{S}$, and this yields good quantum numbers
$\Lambda$, $N$, $S$ and $J$.
...
The splitting of states that we will consider comes from a nuclear spin, which
has operator $\mathbf{I}$. The new total angular momentum is  $\mathbf{F} =
\mathbf{J} + \mathbf{I}$. These states will be important later, since the $m_F$
substates will be used as weak-field seekers for magnetic trapping.
