...
$\mathbf{\tilde{B}}_y$, also shown in (a), to produce the resulting field shown
...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    page) to create the usual field $\mathbf{B}$. This is combined with the
    ...
    shown for $y$ (with $x=0$, $y=h$) in (c) and for $z$ (with $x = y = 0$) in
    (d).
  }
...
and the new bias field, which is parallel to the $x$-axis is labelled
$\mathbf{B}_x$. In the case that $I_1 \ll I$, the second wire can be treated as
a perturbation of the first. The field minimum is therefore still at $h$, and
its depth is \cm{ref earlier equation?}.
...
\cm{Equations for the dimple, including the frequencies, depth, etc.}

% TODO I think the condition here is slightly more complicated, there must be
% some crossover field from minimum to maximum that is not at zero?
...
\begin{figure}[htb]
  ...
  $I_1=I_2=I/10=\SI{0.1}{\milli\ampere}$. Again the bias field is chosen so
...
\cm{Typical implementations, trap frequencies, what can we really achieve
