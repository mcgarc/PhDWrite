...
the assembly of the apparatus. A detailed view is shown in
\myfigref{overview:fig:chipchamber}.
...
initial version, shown in \myfigref{overview:fig:subchip}, does not include
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{figs/exper/exper_4f.pdf}
...
voltage $V_\text{PSU}$ is used to induce a current in the chip wires, with this
...
%The current through the chip can be measured via the potential difference
%across $R_\text{sense} = \SI{0.5}{\ohm}$.
...
a function of supply voltage in \mysubfigref{exper:fig:current}{a}. Also shown
...
in \mysubfigref{exper:fig:current}{b}. The resistivity scales proportionally to
...
obvious solution to this since, as we have explained above, the trapping wires
are fabricated on a single level and are all electrically connected to each
other.  The challenge is to regulate the trapping currents so that any two
wires can be ramped on and off independently of each other. A solution was
developed by Ben Sauer and Noah Fitch of CCM, and is illustrated in
\myfigref{exper:fig:drivers}. One unwanted path of higher resistance is
allowed, and should be eliminated in a future design.

\begin{figure}[htb]
  \centering
  \includegraphics[width=0.6\textwidth]{figs/exper/current_drivers_rot.pdf}
...
In this concept, the current in each wire is controlled by a dedicated current
...
\pewpew{}{00} along with the vibrational repumps, and light is emitted as the
...
in \mysubfigref{exper:fig:exper}{c}. The EM-CCD camera is chosen to be able to
...
results are shown in \myfigref{exper:fig:scatter}, where we have defined the
...
\begin{figure}
  ...
  \begin{tikzpicture}
    \begin{axis}[
        ...
        ylabel=Photons in \SI{100}{\milli\second} ($\times 10^6$),
        width=0.8\textwidth,
        height = 0.4\textwidth,
        legend pos=north east,
        x tick label style={/pgf/number format/.cd, set thousands separator={}},
...
We saw in section~\ref{theory:cooltrap} that the scattering rate for a
...
noted in \inlineref{Shaw2021} that corresponding higher laser power is required
...
in \myeqref{theory:eqn:rabi}, and is reproduced here for clarity
%
\begin{equation}
  \centering
  \hbar \Omega = \bra{g}\mathbf{d}\cdot\mathbf{E}\ket{e}.
  %\tag{\ref{theory:eqn:rabi}}
\end{equation}
...
\end{equation}
%
Additionally we use \myeqref{theory:eqn:IrelE}, to find that the excitation
rate is
%
\begin{equation}
...
For the $0\rightarrow1$ transition, \myeqref{exper:eqn:actualscatterrate}
...
of the laser frequency, we reference \pewpew{}{01} to a \SI{780}{\nano\meter}
...
described in \inlineref{Jurgilas2021}. This infrared (IR) reference light is
combined with the \pewpew{}{01} beam using  a dichroic mirror (DM). This beam
...
cannot resolve individually (see \myfigref{overview:fig:CaFenergy}). We
...
The exposure time is \SI{10}{\milli\second}, following which \pewpew{}{00} is
...
since, as was reported in \inlineref{Shaw2021}, much higher laser power is
