...
molecules as possible. This is then imaged onto a CCD using a second \cm{10cm?}
...
% TODO Did we also use background subtraction?
At first \pewpew{}{00} was used, with a power of \cm{??}.  The height of the
...
The results are shown in \myfigref{exper:fig:scatter}...
%
This figure also shows the background scatter when using \pewpew{}{10} and
\pewpew{}{01} in combination with a \SI{606}{\nano\meter} bandpass filter, this
will be relevant to the background-free imaging scheme described in the next
section.

% NOTE This data is from 2021-11-30_bgfreeimg2.nb
% The export isn't really clean, I manually changed a value in the file and
% added the column headers
\begin{figure}
  \begin{tikzpicture}
    \begin{axis}[
        %ymode=log,
        ...
        ylabel=Predicted photon scatters,
        width=0.8\textwidth,
        height = 0.4\textwidth,
        legend pos=north east,
        x tick label style={/pgf/number format/.cd, set thousands separator={}},
      ...
      \addplot [thick, color=Lorange, only marks, error bars/y explicit, error bars/y dir=both] table [x=x, y=y, y error=err] {figs/exper/bgf606.dat};
      \addlegendentry{\SI{606}{\nano\meter}};
      \addplot [thick, color=Lred, only marks, error bars/y explicit, error bars/y dir=both] table [x=x, y=y, y error=err] {figs/exper/bgf628.dat};
      \addlegendentry{\SI{628}{\nano\meter}};
      \addplot [thick, color=gold, only marks, error bars/y explicit, error bars/y dir=both] table [x=x, y=y, y error=err] {figs/exper/bgf585.dat};
