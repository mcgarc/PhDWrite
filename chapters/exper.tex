...
The laser system for \pewpew{}{01} is shown in \myfigref{exper:fig:laser}. The
beam is collimated to \cm{1/e diameter} and then split on a glass plate. The
majority of the power goes on to the main branch of the system, and the two
pickoffs are directed to the reference cavity (for local locking) and wavemeter
\cm{be more specific}.
%
In the main branch there is an EOM \cm{be more specific about frequency, etc.}, to provide the sidebands which address the
hyperfine levels of the $X$ state. These are the same as the ones discussed in
\cm{section and also paper maybe?} This was not included for the experiment
imgaging the molecule beam \cm{I think}. For experiments in the MOT chamber an
AOM \cm{which one?} in a single-pass configuration to control switching \cm{by
  picking off first order}

\begin{figure}
  \centering
  \cm{Schematic of laser system}
  \caption{}
