...
\cm{Really want to introduce this idea that we can treat the molecule qubit
state as a spin, this needs to be established in mw chapter for single-spin
case.}
...
the spin system.
% TODO Do I follow through on this?
%
...
It will be instructive to visually represent the spin state on a Bloch sphere.
This will be familiar for a single spin, where we write the state in the
form~\cite{PhysRevA.47.3554}
%
\begin{equation}
...
we will limit ourselves to the case where all spins are aligned.  We therefore
have that for all $i$, $\theta_i = \theta$ and $\phi_i = \phi$ and the sphere
has radius $N/2$. Such a state is known as a coherent spin state
(CSS)~\cite{MA201189, Gazeau2009}.

In such a case it is useful to consider a system where all spins are initially
...
% TODO Is the below statement true because the |S, m> states form a basis?
% If so then I can probably say that, otherwise it is a a bit convoluted
It is also useful to denote the state in terms of the eigenstates of the
angular momentum operator. In particular, the eigenstates of $S_z$ ($\ket{S,
m}$) can be used to describe a CSS
%
\begin{equation}
