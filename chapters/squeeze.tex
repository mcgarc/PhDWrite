...
quantities analytically is non-trivial, and will largely depend on experimental
factors, including thermal and electronic noise. However using
\myeqref{squeeze:eqn:homomeas} we can make a good
approximation that for squeezing of width $\Delta_{S_z}$, we require
measurement uncertainty in photon number
%
% TODO Missing this new kappa factor too...
\begin{equation}
...
a function of the probe amplitude ($|\alpha|$). It is now clear that a low
probe intensity is beneficial to the squeezing measurement, with the maximum
squeezing that can be achieved going like $|\alpha|^{-1}$.
