...
%\cm{The process is largely based on that discussed in \inlineref{Treutlein2008}.}
...
\thesis{and model number!}~\cite{} to perform photolithography.
...
\begin{figure}
  \centering
  \ph{Photo of belljar with evaporation cartoon(?)}
  \caption{\ph{caption}}
...
determines/ limits the deposition rates? What happends when we have too much
current?}
...
of Dupont S1805 photoresist. \ph{baking, develop with MF319}

This is placed into the Heidelberg direct writer
for exposure, to create the pattern shown in \ph{fig??}. This pattern is a
simple grid of \ph{2cm by 2cm} squares which will later be diced to become
...
\mysubfigref{fab:fig:prep}{b} and (c) respectively.
...
then be exposed to produce the desired pattern \ph{see fig...} with an exposure
...
produce the trapping wires shown in \ph{figure in section 2}. The moulds,
formed of thick photoresist are positioned above a seed layer of gold, as
depicted in \mysubfigref{fag:fig:prep}{d}. These twelve chips can be divided
...
carries current to the plating contact pad (see \cm{some figure of the
design}). The insulation on the wire prevents an increase in the plating area.
...
etched. This etch uses \ph{some chemical}. The etching rate is \ph{some value}
so \ph{some time} will remove the adhesion layer. The process is the same as
for the gold etch: the chip is submerged in a beaker of the etchant and after
the pre-determined exposure time it is transferred to \ph{DI water} and then
rinsed. This stage of the fabrication is represented in
\myfigref{fab:fig:etch}{d}. 
...
\ph{Example profile (If I have it...)}
