This chapter details the production of our chip traps. Since we require
features with dimensions on the scale of several micrometers, we have employed
...
\SI{6}{\micro\meter} in width. Such small features can be produced using
...
\inlineref{Treutlein2008}, where this value is
...
Next the chip is returned to CCM for through-mask electroplating. It is
important that the dies are treated with great care during transport, and kept
sealed until electroplating can begin. We found that the dies were surprisingly
...
\centering
  ...
  \centering
    ...
    \includegraphics[width=0.28\textwidth]{figs/fab/eplate2.png}
  };
      \begin{scope}[x={(image.south east)},y={(image.north west)}]
...
{ \includegraphics[width=\textwidth]{figs/fab/etchdebris.jpg} };
...
in the same way. For this we use another Sigma-Aldrich pre-mixed etchant of
...
\begin{figure}
  ...
  from the substrate in subfigure (b). This could be due to over-etching or
...
the wirebonds failed before reaching the required current, and therefore were
...
discuss in chapter~\ref{mws} that for a microwave resonator we must fabricate
the CPW with a superconducting material, or else the quality factor will be too
low due to resistive losses. The fabrication of superconducting CPWs is a well
developed field, and is discussed in for example
\inlineref{doi:10.1063/1.3010859}.
