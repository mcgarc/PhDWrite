This chapter details the production of our chip traps using microfabrication
...
outlined in \mytableref{design:table:wires}. Other experiments with two-layer
...
photoresist (blue) is exposed to the UV light by scanning (black lines, not to
scale) of the direct writer. The die is then developed (6) to create a
photoresist mould, which can be electroplated to build tall wires (7). Removing
the mould (8) leaves only the tall wires, and the initial metal layers.
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.22\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figs/fab/bell jar.png}
    \caption{}
  \end{subfigure}
  \caption{
...
the bell jar would then require extensive cleaning after use. It is further
...
Now the chip is taken to the Blackett Laboratory for electroplating.
...
schematically in \mysubfigref{fab:fig:process}{\numrange{5}{8}}.
...
during plating. To measure the plating height we use a stylus profilometer,
...
\SI{10}{\micro\meter}. This also means that the wires can appear to be slightly
...
\mysubfigref{fab:fig:moulds}{a}, meaning that the full wires could not have
...
removing the photoresist mould, see \mysubfigref{fab:fig:aggandcorr}{a}.
...
correctly, shown in \mysubfigref{fab:fig:moulds}{b}. Combined with proper
...
\begin{figure}
  ...
  \caption{
    ...
    the photoresist is removed with Microposit Remover 1165. However the
    smallest wires were damaged by sonicating. The dies shown in (a) and (b)
    are different dies from the same batch. Subfigures (c) and (d) both show
    the same die, which was produced after the cause of both problems was
    identified. All three dies here have been electroplated at
...
pockets in the resin, which could result in virtual leaks under UHV conditions.
%
...
iron will not heat it sufficiently, perhaps due to the aluminium-core PCB
acting as a heat sink. This could be overcome by using a hotplate to heat the
PCB in addition to using a soldering iron to locally achieve high enough
temperatures at the solder join.
...
longer scaling linearly with the number of chips on the wafer. It may be
beneficial to use a smaller wafer, and fabricate a few chips at a time.
Alternatively, the design could be changed so that the traps and
chips are smaller. Etching would be performed on the entire wafer before dicing
into the individual dies.
