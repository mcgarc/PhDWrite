...
the London Centre for Nanotechnology (LCN) cleanroom. I am grateful to the
support staff at LCN for their assistance and support. 
...
$j=\SI{6E10}{\ampere\per\meter\squared}$. Using the wire widths in
\mytableref{design:table:wires}, it is clear that the height required for our
wires is in the trapping $h \sim I w/j = \SI{1}{\ampere} \times
\SI{10}{\micro\meter}/\SI{6E10}{\ampere\per\meter\squared} \sim
\SI{1}{\micro\meter}$.

This can be achieved by through-mask electroplating~\cite{Ruythooren_2000}.
...
layer can be etched away. This is a common technique for constructing atom
chips and is described in \inlinerefs{2011Ac, Lev2003, KOUKHARENKO2004600}.
...
%
\begin{enumerate} \item Dice silicon wafer into \SI{20}{\milli\meter} by
    \SI{20}{\milli\meter} dies.  \item Evaporate adhesion layer of chromium
    ($\sim\SI{10}{\micro\meter}$).  \item Evaporate seed layer of gold
      ($\sim\SI{50}{\micro\meter}$).  \item Spin coat a thick
      (\SI{6}{\micro\meter}) layer of photoresist.  \item Expose photoresist to
      pattern the die.  \item Develop photoresist to create photoresist mould.
      \item Electroplate the chip, such that wires are formed in the mould to
        the desired height.  \item Remove the photoresist mould.  \item
Chemically etch the gold seed layer.  \item Chemically etch the chromium
adhesion layer so as to remove the seed layer and electrically isolate the
wires.  \end{enumerate}
%
...
{\includegraphics[width=0.8\textwidth]{figs/fab/olddesign/oldchip_present.pdf} };
        \begin{scope}[x={(image.south east)},y={(image.north west)}]
        \draw [black, line width=0.2em] (0.3,1.2em) -- node[below,inner
sep=0.1em, font=\footnotesize] {\SI{3}{\milli \meter}} (0.15+0.3,1.2em);
         \end{scope}
      \end{tikzpicture}
    \caption{}
  \end{subfigure}
...
coating of photoresist, although it is possible to coat a \SI{9}{\micro\meter}
if desired, which could be used for plating taller wires. SPR220-7 requires a
post-application bake, first at \SI{90}{\celsius} for two minutes, then
immediately afterwards at \SI{120}{\celsius}.
...
using the Heidlberg DWL 66, a direct writer~\cite{}. 
...
blockages in the moulds. An other indicator that the photoresist has been
successfully removed is if there is good electrical contact through the seed
layer at the wire bond pads, this can be checked with a multimeter.
...
electroplating. Since this does not take place in cleanroom conditions,
but it is important that the dies are treated with great care during transport,
and kept sealed until electroplating can begin. We found that the dies were
surprisingly robust, and were able to be kept for several weeks between
exposure and electroplating.
...
Hold will be deposited in the regions that are not covered by the photoresist
...
\centering
  ...
  \begin{subfigure}[b]{0.22\textwidth}
    \centering
    \includegraphics[width=0.8\textwidth]{figs/fab/eplate.png}
    \caption{}
  \end{subfigure}
  \caption{
...
but we ensure that the entire clip in the solution every time to ensure the
...
With these moulds were able to reliably fabricate features below the
...
%
Examples of photoresist removeal by this procedure are shown in
...
\begin{figure}
  ...
  \caption{
    ...
    \SI{10}{\milli\ampere} for \SI{200}{\milli\second}, and use the early
...
\begin{figure}[h]
  ...
  \end{subfigure}
    \caption{
      ...
      \ph{design 2}. this can be seen as a gradual gradient on the microscope
...
\cm{diammonium hexanitritratocerate} and \numrange{5}{10}\% nitric acid. The
process is the same as for the gold etch: the chip is submerged in a beaker of
the etchant and after the pre-determined exposure time it is transferred to
deionised water and then rinsed. This stage of the fabrication is represented
in \mysubfigref{fab:fig:process}{10}. 
...
\begin{figure}
  ...
  etching. In subfigure (a) we can see that one of the \SI{5}{\micro\meter}
...
\begin{figure}
\centering
  \includegraphics[width=0.4\textwidth]{figs/fab/zeta/wide_crop.jpg}
  \caption{Macroscopic image of a completed chip. The wires are plated to a
  height of \SI{5}{\micro\meter} and the profile of the characterisation lines
  is shown in \mysubfigref{fab:fig:chipprofile}.}
  \label{fab:fig:zeta}
\end{figure}
...
the wirebonds were unsuitable for the current required (see \cm{section in
...
with good thermal conduction.  The two parts of the epoxy mixed in a five to
one ratio by weight using a mass balance, and then a tiny amount is applied to
the subchip. Only enough epoxy is required that when the die is pressed onto
...
and may helping to remove air pockets in the epoxy, which could result in
...
sufficiently high  local temperatures at the solder join.
%
This had the detrimental effect of causing the wirebond pads on the
...
solder \cm{from who?}. This was successful, and useful for our initial current
...
method. Simply soldering with \cm{regular solder} and cleaning the subchip
assembly with IPA was sufficient to reach pressures on the order of
\SI{1E-10}{\milli\bar}, as discussed \cm{in a later chapter}.
