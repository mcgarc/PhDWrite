...
With $\theta_T = \frac{g^2}{\Delta} T$

Expand coherent state from definition
...
\begin{align}
  \ket{\Psi(T)} &= \sum_{m=-S}^S \sum_{n=0}^\infty a_m
  \frac{\alpha^n}{\sqrt{n!}} e^{-i\theta_T S_z a^\dagger a} \ket{n}
  \ket{S, m} \\
  &= \sum_{m=-S}^S \sum_{n=0}^\infty a_m
  \frac{\alpha^n}{\sqrt{n!}} e^{-i\theta_T m n} \ket{n}
  \ket{S, m} \\
  &= \sum_{m=-S}^S a_m \left( \sum_{n=0}^\infty 
...
It is now clear that the state of the system's $S_z$ state is entangled with
the state of the photons. Measuring the phase of the light leaving the cavity
